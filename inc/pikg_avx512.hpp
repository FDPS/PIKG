...
#include <immintrin.h>
  ...
  __m512dx2 _mm512_set1_pdx2(const PIKG::F64vec2 v){
  __m512dx2 ret;
  ret.v0 = _mm512_set1_pd(v.x);
  ret.v1 = _mm512_set1_pd(v.y);
  return ret;
  }
  __m512x2  _mm512_set1_psx2(const PIKG::F32vec2 v){
  __m512x2 ret;
  ret.v0 = _mm512_set1_ps(v.x);
  ret.v1 = _mm512_set1_ps(v.y);
  return ret;
  }
  __m512dx3 _mm512_set1_pdx3(const PIKG::F64vec v){
  __m512dx3 ret;
  ret.v0 = _mm512_set1_pd(v.x);
  ret.v1 = _mm512_set1_pd(v.y);
  ...
  __m512x3  _mm512_set1_psx3(const PIKG::F32vec v){
  __m512x3 ret;
  ret.v0 = _mm512_set1_ps(v.x);
  ret.v1 = _mm512_set1_ps(v.y);
