...
class Kernelprogram
  ...
  def generate_optimized_mncl_kernel_for_mncore2(filename,conversion_type,h = $varhash)
    ...
    ret = `python3 #{File.dirname(__FILE__)}/mncore2_vsm_gen/gen_codehead.py #{filename} && python3 #{File.dirname(__FILE__)}/mncore2_vsm_gen/gen_packs.py #{gen_packs_options}`
    ...
    ni_per_l1b = ((((4096 - 256) / ([io_size[0],io_size[2]].max / 32) / 64)*64)/((simd_width)*64))*(simd_width)*64
    ...
    nj_per_pe = ([256 / (io_size[1]/32), (4*1024*1024/8)/(2*(io_size[1]/32)*4096)].min/simd_width)*simd_width
    ...
    code += "constexpr int MAX_KERNEL_LAUNCH = 64;\n"
    ...
    code += "    delete[]  hEPJ[tag].buffer;\n"
    code += "    hEPJ[tag].buffer = new PIKG::F32[#{io_size[1]/32}*NJMAX*BSIZE*NJ_CLUSTER_MAX[tag]];\n"
    ...
    code += "  status = clEnqueueWriteBuffer(queue, dEPI[tag], CL_FALSE, 0, #{io_size[0]/32}*sizeof(PIKG::F32)*NIMAX*BSIZE, hEPI.buffer, 0, nullptr, &write_events[tag][0]); PIKG_MN_CHECK(status);\n"
    code += "  prof.end(\"WriteEPI\");\n"
    code += "  // NJ SET\n"

    code += "  prof.start(\"WriteEPJFirst\");\n"
    code += "  status = clEnqueueWriteBuffer(queue, dEPJB0[tag], CL_FALSE, 0, #{io_size[1]/32}*sizeof(PIKG::F32)*NJMAX*BSIZE, hEPJ.buffer, 0, nullptr, &write_events[tag][1]); PIKG_MN_CHECK(status);\n"
    fvars.each{ |v|
      iotype   = h[v][0]
      type     = h[v][1]
       fdpsname = h[v][2]
    ...
    nelem = "#{io_size[1]/32}*NJMAX*BSIZE"
    ...
    # size of each elemen may vary. need to be modified for multi precision kernel
    fvars.each{ |v|
      iotype   = h[v][0]
      type     = h[v][1]
      fdpsname = h[v][2]
    ...
    code += "        dEPI[tag] = clCreateBufferWithAttributes(context, CL_MEM_READ_ONLY, 0, sizeof(PIKG::F64)*(tag*DRAM_KERNEL_OFFSET+DRAM_IN_OFFSET), #{io_size[0]/32}*sizeof(PIKG::F32)*NIMAX*BSIZE, nullptr, &status); PIKG_MN_CHECK(status);\n"
    code += "        dEPJB0[tag] = clCreateBufferWithAttributes(context, CL_MEM_READ_ONLY, 0, sizeof(PIKG::F64)*(tag*DRAM_KERNEL_OFFSET+DRAM_IN_J_OFFSET), #{io_size[1]/32}*sizeof(PIKG::F32)*NJMAX*BSIZE*SIZEOF_NJ_CLUSTER, nullptr, &status); PIKG_MN_CHECK(status);\n"
    code += "        dEPJB1[tag] = clCreateBufferWithAttributes(context, CL_MEM_READ_ONLY, 0, sizeof(PIKG::F64)*(tag*DRAM_KERNEL_OFFSET+DRAM_IN_J_OFFSET+DRAM_OFFSET_SIZE), #{io_size[1]/32}*sizeof(PIKG::F32)*NJMAX*BSIZE*SIZEOF_NJ_CLUSTER, nullptr, &status); PIKG_MN_CHECK(status);\n"
    code += "        dFORCE[tag] = clCreateBufferWithAttributes(context, CL_MEM_READ_ONLY, 0, sizeof(PIKG::F64)*(tag*DRAM_KERNEL_OFFSET+DRAM_OUT_OFFSET), #{io_size[2]/32}*sizeof(PIKG::F32)*NIMAX*BSIZE, nullptr, &status); PIKG_MN_CHECK(status);\n"
    code += "      }\n"
    ...
    code += "      status = clEnqueueReadBuffer(queue, dFORCE[i], CL_TRUE, 0, #{io_size[2]/32}*sizeof(PIKG::F32)*NIMAX*BSIZE, hFORCE.buffer, 1, &tail_kernel_event[i], nullptr); PIKG_MN_CHECK(status);\n"
