def get_initial_value(op,type,conversion_type = "reference")
  if op == :plus || op == :minus
    case type
    when /(S|U)/
    #IntegerValue.new("0")
    ...
    elsif conversion_type == "CUDA"
      case type
      when "S64"
        "::cuda::std::numeric_limits<int64_t>::lowest()"
      when "S32"
        "::cuda::std::numeric_limits<int32_t>::lowest()"
      when "S16"
        "::cuda::std::numeric_limits<int16_t>::lowest()"
      when "U64"
        "::cuda::std::numeric_limits<uint64_t>::lowest()"
      when "U32"
        "::cuda::std::numeric_limits<uint32_t>::lowest()"
      when "U16"
        "::cuda::std::numeric_limits<uint16_t>::lowest()"
      when "F64"
        "::cuda::std::numeric_limits<double>::lowest()"
      when "F32"
        "::cuda::std::numeric_limits<float>::lowest()"
      when "F16"
        "::cuda::std::numeric_limits<float16_t>::lowest()"
      else
        abort "error: unsupported type #{type} for get_initial_value"
      end
    end
    ...
    if conversion_type == "reference"
      ...
      case type
      when "S64"
        "::cuda::std::numeric_limits<int64_t>::max()"
      when "S32"
        "::cuda::std::numeric_limits<int32_t>::max()"
      when "S16"
        "::cuda::std::numeric_limits<int16_t>::max()"
      when "U64"
        "::cuda::std::numeric_limits<uint64_t>::max()"
      when "U32"
        "::cuda::std::numeric_limits<uint32_t>::max()"
      when "U16"
        "::cuda::std::numeric_limits<uint16_t>::max()"
      when "F64"
        "::cuda::std::numeric_limits<double>::max()"
      when "F32"
        "::cuda::std::numeric_limits<float>::max()"
      when "F16"
        "::cuda::std::numeric_limits<float16_t>::max()"
      else
        abort "error: unsupported type #{type} for get_initial_value"
      end
    end
